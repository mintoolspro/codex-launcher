...
4. Click a trace row to expand its ordered event timeline. Input, output, reasoning, and tool-call panels use separate color accents. Tool arguments/results and other structured payloads appear as expandable fields; expand **Highlighted JSON** to inspect the sanitized raw JSON, and expand JSON-encoded string fields to drill into their parsed contents. Events named **Visible reasoning**, **Tool call / Tool input**, and **Tool output** contain the provider-returned reasoning and sanitized tool arguments/results. Gateway request traces are listed ahead of standalone Codex telemetry so high-volume OTel events cannot hide them. Use **Refresh** after a turn is complete. **Clear** removes only local trace history.
